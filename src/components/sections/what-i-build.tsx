...
import { Globe, Server, Zap, Sparkles } from "lucide-react";
...
export function WhatIBuild() {
  const categories = [
    ...
    },
    {
      ...
      icon: <Server size={14} className="text-stone-400" />,
    ...
    },
    {
      ...
      icon: <Zap size={14} className="text-stone-400" />,
    ...
    },
    {
      ...
      icon: <Sparkles size={14} className="text-stone-400" />,
  ...
  return (
    <section id="what-i-build" className="relative bg-[#0c0a09] py-6 sm:py-10 border-t border-stone-900">
      <div className="mx-auto max-w-5xl px-6 sm:px-8">
        ...
        {/* Index Table Layout */}
        <div className="grid grid-cols-2 gap-4 md:gap-x-16 md:gap-y-12 border-t border-stone-850 pt-6 sm:pt-12">
          {categories.map((cat) => (
            <div key={cat.title} className="flex flex-col border-b border-stone-900/60 pb-4 md:pb-8 last:border-0 md:last:border-b">
              <div className="flex items-center gap-2 md:gap-3">
                <span className="font-mono text-[9px] md:text-[10px] text-stone-500">{cat.num}</span>
                <span className="shrink-0">{cat.icon}</span>
                <h3 className="text-xs md:text-sm font-serif font-medium text-white">{cat.title}</h3>
              </div>
              <p className="mt-4 text-xs text-stone-300 leading-relaxed hidden sm:block">
                {cat.desc}
              </p>
              
              <div className="mt-5 flex-col gap-1 border-l border-stone-800 pl-3 hidden sm:flex">
